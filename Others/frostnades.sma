...
native sgpm_do_not_frost_me(id);
...
new pcv_enabled, pcv_override, pcv_nadetypes, pcv_teams, pcv_color,
...
new maxPlayers, gmsgCurWeapon, gmsgSetFOV, gmsgScreenFade,
glassGibs, trailSpr, smokeSpr, exploSpr, mp_friendlyfire, czero, bot_quota, czBotHams, fwdPPT, freezetime;
...
new bool:wereStealth[33], bool:inFrost[33], bool:inChill[33];

public plugin_natives(){
	register_library("frostnades_source")
	
	register_native("add_user_immune", "_user_immune", 1)
	register_native("remove_user_immune", "_remove_immune", 1)
	register_native("freeze_player", "_freeze_player", 1)
	register_native("chill_player", "_chill_player", 1)
	register_native("get_state_from_frostnades", "_get_state_from_frostnades", 1)
}

public _user_immune(id)
{
	b_Immune[id] = true
}

public _remove_immune(id){
	b_Immune[id] = false
}

public _get_state_from_frostnades(id){
	if(inFrost[id] || inChill[id]){
		return inFrost[id] ? 2 : 1;
	}
	
	return 0;
}

public _freeze_player(id, nadeTeam, Float:time, Float:variance){
	start_freeze_player(id, nadeTeam, time, variance);
}

public _chill_player(id, nadeTeam, Float:time, Float:variance){
	start_chill_player(id, nadeTeam, time, variance);
}
...
public plugin_init()
{
	...
	pcv_chill_duration = register_cvar("fn_chill_duration","7.5");
	...
	pcv_freeze_duration = register_cvar("fn_freeze_duration","3.5");
...
public frostnade_explode(ent)
{
	...
	for(new target=1;target<=maxPlayers;target++)
	{
		...
		|| (!by_radius && random_num(1,100) <= floatround(radius_calc(distance,FROST_RADIUS,freeze_maxchance,freeze_minchance))))
		{
			...
			start_freeze_player(target, nadeTeam, -1.0, -1.0);
		...
		{
			inChill[target] = true;
			start_chill_player(target, nadeTeam, -1.0, -1.0);
...
start_freeze_player(id, nadeTeam, Float:time, Float:variance)
{
	if(!isFrozen[id])
	{
		//oldGravity[id] = 1.0;
	...
	if(time == -1.0 && variance == -1.0){
		new Float:_variance = get_pcvar_float(pcv_freeze_variance);
		duration = get_pcvar_float(pcv_freeze_duration);
		duration += random_float(-_variance, _variance);
	}else{
		duration = time;
...
start_chill_player(id, nadeTeam, Float:time, Float:variance)
{
	// we aren't already been chilled
	if(!isChilled[id])
	{
		if(!sgcm_in_stealth(id) && !sgpm_do_not_frost_me(id)){
	...
	if(time == -1.0 && variance == -1.0){
		new Float:_variance = get_pcvar_float(pcv_chill_variance);
		duration = get_pcvar_float(pcv_chill_duration)
		duration += random_float(-_variance, _variance);
	}else{
		duration = time;
		duration += random_float(-variance, variance);

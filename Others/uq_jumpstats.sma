native pm_add_user_point_jump(id, points)
...
new bool:player_admin[33],bool:failearly[33],bool:first_onground[33],bool:notjump[33],bool:OnGround[33],bool:donehook[33];
...
public plugin_init()
{
	...
	kz_strafe_y      = register_cvar("kz_uq_strafe_y",      "0.45");
		...
		else
		{
			...
			if( g_alive[id] && team[id]==uq_team)
			{
				...
				{	
					...
					// G I V E P O I N T S F O R J U M P
					if( !failed_jump[id] && get_pcvar_num(agm_status) >= 1 && get_pcvar_num(agm_status) <= 2)
					{
						if((jump_type[id]==Type_Bhop_In_Duck || jump_type[id]==Type_Up_Bhop_In_Duck || jump_type[id]==Type_Up_Stand_Bhop || jump_type[id]==Type_Up_Bhop || jump_type[id]==Type_Real_ladder_Bhop || jump_type[id]==Type_Double_CountJump
							|| (multiscj[id]!=2 && jump_type[id]==Type_StandUp_CountJump) || (multidropcj[id]!=2 && jump_type[id]==Type_Drop_CountJump) || jump_type[id]==Type_CountJump
							|| jump_type[id]==Type_Drop_BhopLongJump || jump_type[id]==Type_BhopLongJump || jump_type[id]==Type_StandupBhopLongJump || jump_type[id]==Type_WeirdLongJump
							|| jump_type[id]==Type_ladderBhop || jump_type[id]==Type_ladder || jump_type[id]==Type_LongJump || jump_type[id]==Type_HighJump))
						{
							if ( distance[id] >= god_dist ) 
							{
								if(get_user_team(id) == 1 && get_playersnum() > 3)
								{
									new points = 3;
									pm_add_user_point_jump(id, points);
									Color_Chat_Lang(0, RED, "%L",LANG_SERVER,"UQSTATS_GIVE_AP",g_playername[id], points);
								}
								else if(get_playersnum() < 4)
								{
									Color_Chat_Lang(id, RED, "%L",LANG_SERVER,"UQSTATS_NO_PLAYERS");
								}
							}
							else if ( distance[id] >= leet_dist  ) 
							{
							...
							else if ( distance[id] >= holy_dist ) 
							{
								if(get_user_team(id) == 1 && get_playersnum() > 3)
								{
									new points = 1;
									pm_add_user_point_jump(id, points);
									Color_Chat_Lang(0, BLUE, "%L",LANG_SERVER,"UQSTATS_GIVE_1_AP",g_playername[id], points);
								}
								else if(get_playersnum() < 4)
								{
									Color_Chat_Lang(id, BLUE, "%L",LANG_SERVER,"UQSTATS_NO_PLAYERS");
								}
							}
						}
						else if(jump_type[id]==Type_Multi_CountJump || (multiscj[id]==2 && jump_type[id]==Type_StandUp_CountJump) || (multidropcj[id]==2 && jump_type[id]==Type_Drop_CountJump))
